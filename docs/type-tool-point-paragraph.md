...
- **Box align (L/C/R/J):** same attrKey closure fix unblocks paragraph buttons. Layout already offsets wraps inside `layer.width`; align updates **only** `params.halign` (frame W/H/Mode unchanged). Justify enabled for boxes. `normalize_text_boundary` / `is_box_text` keep UI “Paragraph” from being persisted; details panel + options-bar Mode sync use the same canonical `box`/`dynamic`.
- **Weight list for multi-face families** must enumerate Local Font Access styles + Google/user catalog variants (e.g. Roboto Thin…Black), not a single Regular fallback. Empty local variants return `[]` so Google/user still merge; Weight change syncs Bold (≥600) and re-syncs Mode/align from the layer before the options bar remounts.
- **Box resize (frame-only):** Move (`select.js`) and Type handle resize for `boundary: box` update **only** `x/y/w/h`. Shared `is_box_text` gates out `commit_point_text_resize` / live `scale_*` so paragraph never bakes like point text.
