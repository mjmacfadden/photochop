...
class GUI_tools_class {
	...
	render_transform_attributes(container) {
		...
		// When a text layer is selected, show Size so Select-transform live/bake sync
		// can update the same options-bar control Text tool uses (.item.size).
		if (layer && layer.type === 'text') {
			const textToolCfg = (config.TOOLS || []).find((t) => t && t.name === 'text');
			const sizeAttr = textToolCfg && textToolCfg.attributes ? textToolCfg.attributes.size : null;
			let sizeValue = 38;
			if (sizeAttr != null) {
				sizeValue = (typeof sizeAttr === 'object' && sizeAttr.value != null) ? sizeAttr.value : sizeAttr;
			}
			try {
				const span0 = layer.data && layer.data[0] && layer.data[0][0] ? layer.data[0][0] : null;
				if (span0 && span0.meta && span0.meta.size != null && isFinite(Number(span0.meta.size))) {
					// Layer baked size is source of truth on bar rebuild; live drag updates DOM via _sync_size_attribute
					sizeValue = Number(span0.meta.size);
				}
			} catch (e) { /* ignore */ }
			if (!isFinite(Number(sizeValue))) sizeValue = 38;
			sizeValue = Math.round(Number(sizeValue) * 100) / 100;

			const sizeItem = document.createElement('div');
			sizeItem.className = 'item size';

			const sizeLabel = document.createElement('label');
			sizeLabel.className = 'trn';
			sizeLabel.id = 'attribute_label_size';
			sizeLabel.htmlFor = 'size_input';
			sizeLabel.innerHTML = 'Size:';

			const sizeInput = document.createElement('input');
			sizeInput.type = 'number';
			sizeInput.setAttribute('aria-labelledby', 'attribute_label_size');
			const min = (sizeAttr && typeof sizeAttr === 'object' && sizeAttr.min != null) ? sizeAttr.min : 1;
			const max = (sizeAttr && typeof sizeAttr === 'object' && sizeAttr.max != null) ? sizeAttr.max : 999;
			const step = (sizeAttr && typeof sizeAttr === 'object' && sizeAttr.step != null) ? sizeAttr.step : 1;
			const inputStep = (sizeAttr && typeof sizeAttr === 'object' && sizeAttr.inputStep != null) ? sizeAttr.inputStep : 0.01;
			const inputType = (sizeAttr && typeof sizeAttr === 'object' && sizeAttr.inputType) ? sizeAttr.inputType : 'text';

			const $numberInput = $(sizeInput)
				.uiNumberInput({
					id: 'size',
					inputId: 'size_input',
					name: 'size',
					...
					inputStep,
					inputType,
					exponentialStepButtons: false
				})
				.on('input', () => {
					let value = $numberInput.uiNumberInput('get_value');
					if (value == null || !isFinite(value)) return;
					// Keep Text tool attrs in sync so switching to Text shows the same size
					if (textToolCfg && textToolCfg.attributes && textToolCfg.attributes.size) {
						if (typeof textToolCfg.attributes.size === 'object') textToolCfg.attributes.size.value = value;
						else textToolCfg.attributes.size = value;
					}
					const textTool = this.tools_modules['text'] && this.tools_modules['text'].object;
					if (textTool && typeof textTool.on_params_update === 'function') {
						textTool.on_params_update({ key: 'size', value });
					}
				});

			sizeItem.appendChild(sizeLabel);
			sizeItem.appendChild($numberInput[0]);
